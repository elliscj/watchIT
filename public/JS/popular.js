$(document).ready(function () {
  ...
  function getpopularData() {
    $.getJSON(popularURL, function (popularData) {
      // console.log(popularData);
      //we needed to add .results because popularData is an array.
      for (let i = 0; i < popularData.results.length; i++) {
        // w300 is how wide it is
        var mid = popularData.results[i].id;
        // mid = movie ID
        var thisMovieUrl =
        ...
        $.getJSON(thisMovieUrl, function (movieKey) {
          ...
          var poster =
            imageBaseUrl + "w300" + popularData.results[i].poster_path;
          // console.log(poster);

          var title = popularData.results[i].original_title;

          var releaseDate = popularData.results[i].release_date;

          var overview = popularData.results[i].overview;
          // $('.overview').addClass('overview');

          var voteAverage = popularData.results[i].vote_average;
          // console.log(movieKey)
          ...
          popularHTML += '<div class="modal-dialog" role="document">';
          ...
          // popularHTML += '<div class="genre">Genre: '+genre+'</div><br>';
          popularHTML += '<div class="overview">' + overview + "</div><br>"; // Put overview in a separate div to make it easier to style
          popularHTML +=
            '<div class="rating">Rating: ' + voteAverage + "/10</div><br>";
          popularHTML += `<div class="col-sm-5 btn btn-primary add-to-favorites" data-overview="${overview}" data-poster="${poster}" data-trailer="${youtubeLink}" data-title="${title}">Add to Favorite</div>`;

          popularHTML += "</div>"; //close movieDetails
          popularHTML += "</div>"; //close modal-content
          popularHTML += "</div>"; //close modal-dialog
          popularHTML += "</div>"; //close modal
          popularHTML += "</div>"; //close off each div

          $("#movie-grid").append(popularHTML);
  ...
  // call getMoviesByGenre using click function but call getpopularData on default.
  getpopularData();
